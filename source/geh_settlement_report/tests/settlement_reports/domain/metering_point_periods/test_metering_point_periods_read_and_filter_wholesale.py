...
import tests.settlement_reports.test_factories.metering_point_periods_factory as metering_point_periods_factory
...
def test_read_and_filter__when_balance_responsible_party_changes_on_metering_point__returns_single_period(
    ...
    assert actual.select(DataProductColumnNames.from_date).collect()[0][0].replace(tzinfo=timezone.utc) == d.JAN_1ST
    assert actual.select(DataProductColumnNames.to_date).collect()[0][0].replace(tzinfo=timezone.utc) == d.JAN_3RD
...
def test_read_and_filter__when_datahub_user_and_energy_supplier_changes_on_metering_point__returns_two_link_periods(
    ...
    assert actual_row_1[DataProductColumnNames.from_date].replace(tzinfo=timezone.utc) == d.JAN_1ST
    assert actual_row_1[DataProductColumnNames.to_date].replace(tzinfo=timezone.utc) == d.JAN_2ND

    actual_row_2 = actual.collect()[1]
    assert actual_row_2[DataProductColumnNames.energy_supplier_id] == es_id_b
    assert actual_row_2[DataProductColumnNames.from_date].replace(tzinfo=timezone.utc) == d.JAN_2ND
    assert actual_row_2[DataProductColumnNames.to_date].replace(tzinfo=timezone.utc) == d.JAN_3RD
