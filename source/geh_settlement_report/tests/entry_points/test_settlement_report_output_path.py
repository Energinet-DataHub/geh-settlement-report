...
def _assert_args(args, args_dict, env_args):
    ...
    assert args.settlement_reports_output_path == env_args["SETTLEMENT_REPORTS_OUTPUT_PATH"]

    # Differentiate between balance fixing and other calculation types
    if args.calculation_type == CalculationType.BALANCE_FIXING:
        assert args.grid_area_codes == [str(i) for i in json.loads(args_dict["grid-area-codes"])]
    else:
        assert {k: str(v) for k, v in args.calculation_id_by_grid_area.items()} == json.loads(
            args_dict["calculation-id-by-grid-area"]
        )

    # Optional arguments
    if args.energy_supplier_ids:
        assert args.energy_supplier_ids == [str(v) for v in json.loads(args_dict.get("energy-supplier-ids"))]
    assert args.split_report_by_grid_area is args_dict.get("split-report-by-grid-area", False)
    assert args.prevent_large_text_files is args_dict.get("prevent-large-text-files", False)
...
def test_grid_area_code_validation(reason, args, environment_variables: dict, monkeypatch: pytest.MonkeyPatch):
