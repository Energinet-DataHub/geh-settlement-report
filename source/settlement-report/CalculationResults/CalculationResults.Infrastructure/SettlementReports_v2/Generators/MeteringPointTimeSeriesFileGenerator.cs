...
using Energinet.DataHub.SettlementReport.CalculationResults.Interfaces.CalculationResults.Model;
...
namespace Energinet.DataHub.SettlementReport.CalculationResults.Infrastructure.SettlementReports_v2.Generators;

public sealed class MeteringPointTimeSeriesFileGenerator : ISettlementReportFileGenerator
{
    private const int ChunkSize = 65_000; // About 31 rows per day, 251.875 rows in total.
        ...
        StreamWriter destination)
    {
        ...
        await using (csvHelper.ConfigureAwait(false))
        {
            ...
            var rowsCount = 0;
            await foreach (var record in _dataSource.GetAsync(filter, maximumCalculationVersion, _resolution, fileInfo.FileOffset * ChunkSize, ChunkSize).ConfigureAwait(false))
            {
